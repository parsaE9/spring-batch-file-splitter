...
public class PspBatchItemWriter {
    ...
    private FlatFileItemWriter<BatchRecord> createWriter(SwitchDto switchDto) throws Exception {
        String pspOutputPath = new File(outputDirectoryPath + "/PSPs/" + switchDto.getFolderName() + "/" + "batch_" + UnzipService.fileDate + "_cycle_01_details.shap_psp_" + switchDto.getIin() + ".txt").getAbsolutePath();
        switchDto.setFilePath(pspOutputPath);
